...
// Project 3.5_memory mapped files
#include <string.h>
...
// the table of function pointers for file-backed pages
...
lazy_load_file (struct page* page, void* aux){
	...
	page -> file.size = file_read (mi->file, page->va, mi->read_bytes); // load 하기
	page -> file.ofs = mi->offset;
	if (page->file.size != PGSIZE){
		memset (page->va + page->file.size, 0, PGSIZE - page->file.size);
	}
	pml4_set_dirty (thread_current()->pml4, page->va, false); // dirty bit 바뀜
	free(mi);
	return true;
}
	...
	for (struct list_elem* i = list_front (&mmap_file_list); i != list_end (&mmap_file_list); i = list_next (i))
	{
		...
		if (mfi -> start == (uint64_t) addr){
			for (uint64_t j = (uint64_t)addr; j<= mfi -> end; j += PGSIZE){
				...
				spt_remove_page(&thread_current()->spt, page); // munmap
			}
